 ...
 *
 */
(this["webpackChunkgethue"] = this["webpackChunkgethue"] || []).push([["calcite-parser~dasksql-parser~druid-parser~elasticsearch-parser~-591727303"],{
...
var initSyntaxParser = function initSyntaxParser(parser) {
  ...
  parser.parseSyntax = function (beforeCursor, afterCursor, debug) {
    ...
    parser.yy.latestTablePrimaries = [];
    parser.yy.activeDialect = 'generic'; // TODO: Find a way around throwing an exception when the parser finds a syntax error
    ...
    if (parser.yy.error && (parser.yy.error.loc.last_column < beforeCursor.length || !beforeCursor.endsWith(parser.yy.error.text))) {
      ...
      for (var i = 0; i < parser.yy.error.expected.length; i++) {
        var expected = parser.yy.error.expected[i]; // Strip away the surrounding ' chars

        expected = expected.substring(1, expected.length - 1); // TODO: Only suggest alphanumeric?
...
//# sourceMappingURL=/static/desktop/js/bundles/workers/calcite-parser~dasksql-parser~druid-parser~elasticsearch-parser~-591727303-chunk-8928692e45d8ba061ae3.js.map